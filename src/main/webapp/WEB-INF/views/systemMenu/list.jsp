		...
		<div id="container">
			...
			<div class="ui_content">
				<div>
					...
					<c:forEach var="menu" items="${parents}"><%--设置子标签--%>
						-><a href="/systemMenu/list.do?parenId=${menu.id}">${menu.name}</a>
					</c:forEach>
				...
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach items="${list}" var="entity" varStatus="num">
							<tr>
								<td><input type="checkbox" class="acb" /></td>
								<td>${num.count}</td>
								<td>${entity.name}</td>
								<td>${entity.sn}</td>
								<td>${entity.parent.name}</td><%--有错误记得检查这里--%>
								<td>${entity.url}</td>
								<td>
